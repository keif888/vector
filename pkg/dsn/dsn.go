...
// dsnKeyValuePasswordPattern is a regular expression matching a password or api-key in a key=value database DSN string.
var dsnKeyValuePasswordPattern = regexp.MustCompile(`(?i)(password\s*=\s*|api-key\s*=\s*)("[^"]*"|'[^']*'|\S+)`)
...
func (d *DSN) MaskPassword() (s string) {
	...
	if d.Driver == DriverPostgres || d.Driver == DriverQdrant || strings.Contains(s, " password") || strings.Contains(s, " api-key") {
		return dsnKeyValuePasswordPattern.ReplaceAllStringFunc(s, func(segment string) string {
			matches := dsnKeyValuePasswordPattern.FindStringSubmatch(segment)
			if len(matches) != 3 {
				return segment
			}
...
func (d *DSN) parse() bool {
	...
	default:
		if matches := dsnPattern.FindStringSubmatch(d.DSN); len(matches) > 0 {
			...
			d.detectDriver() // Make sure that the driver is still valid.
			return true
		}
	}
	return false
...
// parseQdrant extracts connection settings from "Qdrant" key/value style DSNs and
// returns true on success.
func (d *DSN) parseQdrant() bool {
	if strings.Contains(d.DSN, "password=") || strings.Contains(d.DSN, "user=") {
		return false
	}
	...
	d.Driver = DriverQdrant
	d.User = ""
	d.Password = values["api-key"]
	d.Name = ""
...
func (d *DSN) detectDriver() {
	...
	switch {
	case d.Net == "tcp" || d.Net == "unix" || strings.Contains(lower, "@tcp(") || strings.Contains(lower, "@unix("):
		d.Driver = DriverMySQL
	case strings.HasPrefix(lower, "file:") || strings.HasSuffix(lower, ".db") || strings.HasSuffix(strings.ToLower(d.Name), ".db"):
		d.Driver = DriverSQLite3
	case strings.Contains(lower, "api-key="):
		d.Driver = DriverQdrant
	case strings.Contains(lower, "postgres://") || strings.Contains(lower, "postgresql://"):
		d.Driver = DriverPostgres
	case strings.Contains(lower, " host=") || strings.Contains(lower, " port=") || strings.Contains(lower, " dbname=") || strings.Contains(lower, " database="):
		d.Driver = DriverPostgres
	case d.Server != "" && (strings.Contains(d.Server, ":") || d.Net != "") && d.Driver == "":
		d.Driver = DriverMySQL
	}
}
...
func (d *DSN) ToString() string {
	...
	case DriverQdrant:
		if d.Params != "" {
			return fmt.Sprintf("host=%s port=%d api-key=%s %s", d.Host(), d.Port(), d.Password, d.Params)
		} else {
			return fmt.Sprintf("host=%s port=%d api-key=%s %s", d.Host(), d.Port(), d.Password, Params[DriverQdrant])
		}
	default:
		return ""
	}
